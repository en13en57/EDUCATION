package kr.green.maven.java211217;
...
그러면, 주식의 거래데이터가 아래와 같이 주어질 때 를 만듭니다.
		...
		try {
			...
			for(int i=0;i<data.size();i++ ) { 
				...
				if(!oldStr.equals(temp.split(":")[0])) { // temp.split(":")[0] : 데이터중 분만 취득=> 분이 바뀌었다면.
					openList.add(Integer.parseInt(temp.split(",")[1].trim())); // open값 저장
					oldStr = temp.split(":")[0]; // 현재 분을 oldStr에 저장
					if(i>0) { // 맨 첫번째는 제외.
						// 분이 바뀌었을때 바로 이전의 값
						closeList.add(Integer.parseInt(data.get(i-1).split(",")[1].trim())); // i가 open 값이니, i-1값이 close 값.
						// 분이 바뀌었을때 최대/최소값 저장 (분당 최대,최소)
						highList.add(max);
						lowList.add(min);
					}
					// 분이 바뀌었을때 최대/최소값 초기화
			...
			// open값은 저장이 되었지만 high/low/close의 경우 분이 바뀌어야 저장이 되므로, 마지막 분의경우 최대/최소/최종 값은 별도로 저장해 주어야 한다.
			highList.add(max);
