...
public class Exam02 {
	...
	public static void reverseAndAdd(long n) {
		int count = 0;
		do {
			n = backAdd(n); // 뒤집어서 더하기 반복
			count ++; // 반복횟수 증가
		}while(!isPalindrome(n)); // 회문이면 종료
		System.out.println(count + " " + n); // 반복횟수와 회문인 숫자 출력
	}
	
	// 뒤집어서 더한 값을 리턴하는 메서드
	public static long backAdd(long n) {
		return n + Long.parseLong(new StringBuffer(n+"").reverse().toString());
	}
	
	// 회문(palindrome, 앞뒤 어느 쪽에서 읽어도 같은 말이 되는 어구) 판단해주는 메서드
